...
if(isset($save))
{
	...
	{
		$op=md5($op);
$sql=mysqli_query($conn,"select * from admins where pass='$op'");
...
if($r==true)
{
	...
	{
	$np=md5($np);
	$sql=mysqli_query($conn,"update admins set pass='$np' where email='$admin'");
	
	$err="<font color='yellow'>Password updated </font>";
	}
	else
	{
